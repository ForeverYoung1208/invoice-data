      ...
      await Promise.all(
        files.map(async (file) => {
          try {
            ...
            newErrors[file.id] = err instanceof Error ? err.message : 'Failed to load content';
          }
        })
      );
                  ...
                  <TableHeader className="bg-slate-50">
                    <TableRow>
                      {Object.keys(contents[file.id][0]).map((header) => (
                        <TableHead key={header} className="text-xs text-slate-600 whitespace-nowrap">
                          {header}
                        </TableHead>
                      ))}
                    </TableRow>
                    ...
                    {contents[file.id].map((row, i) => (
                      <TableRow key={i}>
                        {Object.values(row).map((val: any, j) => (
                          <TableCell key={j} className="text-xs text-slate-700 py-1 whitespace-nowrap">
                            {String(val)}
                          </TableCell>
                        ))}
                      </TableRow>
